...
namespace SmartTooling_API.Controllers
{
    ...
    public class ModelOperationController : ControllerBase
    {
        ...
        [HttpPost("allmodel-operation")]
        public async Task<IActionResult> GetAllModelOperation([FromQuery] PaginationParams param)
        {
            var lists = await _modelOperationService.GetAllModelOperation(param);
            Response.AddPagination(lists.CurrentPage, lists.PageSize, lists.TotalCount, lists.TotalPages);
            return Ok(lists);
        }
        [HttpGet("model-no")]
