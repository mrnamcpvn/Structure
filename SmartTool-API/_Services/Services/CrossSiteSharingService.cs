...
namespace SmartTool_API._Services.Services
{
    ...
    {
        private readonly IKaizenBenefitsApplicationFormRepository _formRepo;
        ...
        public CrossSiteSharingService(
            IKaizenBenefitsApplicationFormRepository formRepo,
        ...
        {
            var form = _formRepo.FindAll(x => x.to_factory_id.Trim() == factory.Trim() && x.model_no.Trim() == modelNo && x.serial_no == serialNo.ToInt());
            var model = _repoModel.FindAll();
            var kaizen = _kaizen.FindAll();
            var operation = _modelOperationRepository.FindAll();
         ...
         b in model on new { a.factory_id, a.model_no } equals new { b.factory_id, b.model_no }
                              ...
                              join d in operation on new { c.factory_id, c.model_no, c.operation_id } equals
        ...
        public async Task<List<CrossSiteSharingEditDTO>> GetCrossSiteSharingPDF(List<CrossSiteSharingDTO> filterParam)
        {
            ...
            var form = await _formRepo.FindAll().ToListAsync();
            var model = _repoModel.FindAll();
            var kaizen = _kaizen.FindAll();
        ...
        public async Task<PagedList<CrossSiteSharingDTO>> Search(PaginationParam param, CrossSiteSharingParam filterParam)
        {
            var form = _formRepo.FindAll();
            var model = _repoModel.FindAll();
            var kaizen = _kaizen.FindAll();
            ...
            if (!string.IsNullOrEmpty(filterParam.model_no))
                model = model.Where(x => x.model_no.Contains(filterParam.model_no) || x.model_name.Contains(filterParam.model_no));
        ...
        public async Task<string> GetdocNo()
        {
            ...
            var item = await _formRepo.FindAll(x => x.doc_no.Contains(doc_no)).OrderByDescending(x => x.doc_no).FirstOrDefaultAsync();
            if (item != null)
            {
                var seri_number = item.doc_no.Substring(8).ToInt();
                var tmp = (seri_number >= 99) ? (seri_number + 1).ToString() : (seri_number < 9) ? ("00" + seri_number + 1) : ("0" + seri_number + 1);
        ...
        public async Task<OperationResult> UpdateCrossSiteSharing(Kaizen_Benefits_Application_FormDTO model)
        {
            ...
            _formRepo.Update(modelUp);
            await _formRepo.SaveAll();
