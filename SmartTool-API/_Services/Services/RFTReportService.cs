...
namespace SmartTool_API._Services.Services
{
    public class RFTReportService : IRFTReportService
    {
        ...
        public async Task<PagedList<Model>> SearchRFTReport(PaginationParams paginationParams, RFTReportParam rftReportParam)
        {
            ...
            if (!String.IsNullOrEmpty(rftReportParam.factory_id))
            {
                data = data.Where(x => x.factory_id.Trim() == rftReportParam.factory_id.Trim())
                .OrderByDescending(x => x.prod_season).ThenByDescending(x => x.volume).ToList();
            }
            if (!String.IsNullOrEmpty(rftReportParam.model_no))
            {
                data = data.Where(x => x.model_no.Contains(rftReportParam.model_no) || x.model_name.Contains(rftReportParam.model_no))
                .OrderByDescending(x => x.prod_season).ThenByDescending(x => x.volume).ToList();
            }
        ...
        public async Task<List<VW_RFTReportDetailDTO>> SearchRFTReportDetail(RFTReportParam rftReportParam)
        {
            ...
            var data = await _repoViewRFTReport.FindAll(x => x.factory_id.Trim() == rftReportParam.factory_id.Trim() &&
             x.model_no == rftReportParam.model_no.Trim())
        ...
        public async Task<object> GetAVG(string factory_id,  string model_no)
        {
            ...
            { avgdata.cr2, avgdata.sms, avgdata.cs1, avgdata.cs2, avgdata.cs3, 
            avgdata.prod1, avgdata.prod2, avgdata.mp1, avgdata.mp2, avgdata.mp3 };
