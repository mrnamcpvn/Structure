...
export class RftDetailComponent implements OnInit {
  ...
  getRFTReportDetail() {
    this.rftReportService.searchRFTReportDetail(this.model).subscribe(res => this.rftReportDetails = res);
  }

  getAVG() {
    this.rftReportService.getAverage(this.model.factory_id, this.model.model_no)
      .subscribe(res => {
        this.avgdata = res;
