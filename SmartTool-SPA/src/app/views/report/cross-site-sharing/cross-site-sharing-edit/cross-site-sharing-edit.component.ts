...
export class CrossSiteSharingEditComponent implements OnInit {
  ...
  loaddata() {
    this.crosssitesharingSerivce
      .getCrossSiteSharingEdit(this.factory, this.model_no, this.serialNo)
      .subscribe(
        (res) => {
  ...
  save() {
    if (
      this.models.benefits_category_hse == false &&
      this.models.benefits_category_quality == false &&
      this.models.benefits_category_delivery == false &&
      this.models.benefits_category_efficiency == false &&
      this.models.benefits_category_others == false
    ) {
      this.alertify.error("Please Choise Benefits Category");
    } else {
      this.crosssitesharingSerivce
        .UpdateCrossSiteSharing(this.models)
        .subscribe(
          (res) => {
            if (res.success) {
