...
export class CrossSiteSharingEditComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute,
    ...
    private crossSiteSharing: CrossSiteSharingService
  ) { }
  ...
  loadData() {
    this.spinner.show();
    this.crossSiteSharing.getCrossSiteSharingEdit(this.factory, this.model_no, this.serialNo).subscribe(res => {
      ...
      this.spinner.hide();
    }, error => this.snotify.error("Have Error"));
  }
  ...
  save() {
    if (this.models.benefits_category_hse == false && this.models.benefits_category_quality == false
      ...
      this.snotify.error("Please choose Benefits Category");
    else {
      this.crossSiteSharing.UpdateCrossSiteSharing(this.models).subscribe(
        res => {
          if (res.success) {
