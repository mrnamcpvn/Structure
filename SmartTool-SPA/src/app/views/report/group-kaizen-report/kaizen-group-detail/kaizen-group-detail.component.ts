...
import { KaizenBenefitsApplicationForm } from "../../../../_core/_model/kaizen-benefits-application-form";
...
export class KaizenGroupDetailComponent implements OnInit {
  ...
  constructor(
    ...
    private groupKaizenReportService: GroupKaizenReportService,
    private spinner: NgxSpinnerService,
    private alertify: AlertifyService
  ) {}

  ngOnInit() {
    this.groupKaizenReportService.currentKaizen.subscribe(
      (res) => (this.kaizen = res)
    );
    if (this.kaizen !== null) {
  ...
  getKaizenDetail() {
    this.groupKaizenReportService
      .getKaizenDetail(
        this.kaizen.factory_id,
        this.kaizen.model_no,
        this.kaizen.serial_no
      )
      .subscribe((res) => {
        this.kaizenDetail = res;
        this.kaizenDetail.kaizen.before_remarks =
          this.functionUtility.replaceLineBreak(
            this.kaizenDetail.kaizen.before_remarks
          );
        ...
        if (
          !this.functionUtility.isEmpty(this.kaizenDetail.kaizen.before_media)
        ) {
        ...
        if (
          !this.functionUtility.isEmpty(this.kaizenDetail.kaizen.after_media)
        ) {
  ...
  addCross() {
    this.alertify.confirm(
      "Cross Site Sharing",
      "This action cannot be undo. Please confirm.",
      () => {
        ...
        this.spinner.show();
        this.groupKaizenReportService
          .addCross(kaizenBenefitsApplicationForm)
          .subscribe(
            (res) => {
              if (res.success) {
                this.spinner.hide();
  ...
  getFactory() {
    this.groupKaizenReportService.getFactory().subscribe((res) => {
