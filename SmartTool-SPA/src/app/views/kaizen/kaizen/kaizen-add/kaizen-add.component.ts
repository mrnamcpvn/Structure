...
import { KaizenQuery } from '../../../../_core/_queries/kaizen.query';
...
export class KaizenAddComponent implements OnInit {
  ...
  constructor(private _kaizenService: KaizenService,
    ...
    private kaizenQuery: KaizenQuery
  ) { }
  ...
  // queryKaizen() {
  //   this.kaizenQuery.selectAll().subscribe((models) => (this.dataKaizen = models));
  // }
  ...
  save() {
    ...
    this._kaizenService.create(this.dataKaizen).subscribe(
      res => {
        if (res.success) {
          this.spinner.hide();
          this.alertify.success('Add Success', 'Success');
          this.router.navigate(['/kaizen/kaizen/']);
        } else {
          this.alertify.error('Add Fail', 'Error');
          this.spinner.hide();
          return;
        }
      },
  ...
  setData() {
    ...
    this.dataKaizen.kaizen_type_eliminate =
                    this.dataKaizen.kaizen_type_eliminate == undefined ? false : this.dataKaizen.kaizen_type_eliminate;
    ...
    this.dataKaizen.kaizen_type_smart_tool =
                    this.dataKaizen.kaizen_type_smart_tool == undefined ? false : this.dataKaizen.kaizen_type_smart_tool;
  }
  cancel() {
    this.dataKaizen = new Kaizen;
    this.stage = '';
