...
export class AddComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.modelQuery.select(state => state.modelTypes).pipe(untilDestroyed(this)).subscribe(modelTypes => {
  ...
  save(model: Model) {
    ...
    model.create_by = JSON.parse(localStorage.getItem('user')).name;
    model.update_by = JSON.parse(localStorage.getItem('user')).name;
    model.create_time = new Date(Date());
    model.update_time = model.create_time;
    this.modelService.createModel(model).pipe(untilDestroyed(this)
    ).subscribe(() => {
